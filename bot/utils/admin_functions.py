...
from database_functions.giveaway_tickets_db import (
    get_all_tickets_for_export,
    get_tickets_buyers_count,
    get_tickets_sold_count,
    get_tickets_top_buyers,
)
...
def format_statistics_message():
    ...
    giveaway = stats.get('giveaway', {})
    giveaway_subscribed = giveaway.get('subscribed', 0)
    giveaway_reposted = giveaway.get('reposted', 0)
    giveaway_completed = giveaway.get('completed', 0)
    total = stats['total_users']
    sub_pct = (giveaway_subscribed / total * 100) if total > 0 else 0
    rep_pct = (giveaway_reposted / total * 100) if total > 0 else 0
    comp_pct = (giveaway_completed / total * 100) if total > 0 else 0

    message = f"""<b>📊 СТАТИСТИКА — BMW M4 РОЗІГРАШ</b>

<b>◼ РОЗІГРАШ</b>
• Підписались на канали: <b>{giveaway_subscribed}</b> ({sub_pct:.1f}%)
• Зробили репост: <b>{giveaway_reposted}</b> ({rep_pct:.1f}%)
• Виконали всі умови: <b>{giveaway_completed}</b> ({comp_pct:.1f}%)

<b>◼ КОРИСТУВАЧІ</b>
• Всього: <b>{stats['total_users']}</b>
• Сьогодні: <b>{stats['new_today']}</b>
• За тиждень: <b>{stats['new_week']}</b>
• За місяць: <b>{stats['new_month']}</b>

<b>◼ АКТИВНІСТЬ</b>
• Сьогодні активних: <b>{stats['active_today']}</b>
...
• Переходів: <b>{stats['total_clicks']}</b>
• Прийшли з посилань: <b>{stats['users_from_links']}</b> ({links_percentage:.1f}%)
...
def _format_top_buyer_line(rank: int, uid: int, cnt: int, uname: str) -> str:
    """Посилання tg://user?id=… для відкриття профілю в Telegram; за наявності — t.me/username."""
    id_link = f'<a href="tg://user?id={uid}">ID {uid}</a>'
    u = (uname or "").strip().lstrip("@")
    if u:
        user_bit = f'{id_link} · <a href="https://t.me/{u}">@{escape(u)}</a>'
    else:
        user_bit = id_link
    return f"\n   {rank}. {user_bit}: <b>{cnt}</b> шт."


def format_tickets_statistics_message() -> str:
    sold = get_tickets_sold_count()
    buyers = get_tickets_buyers_count()
    total = giveaway_total_tickets
    remaining = max(0, total - sold)
    pct_sold = (sold / total * 100) if total > 0 else 0.0
    pct_rem = (remaining / total * 100) if total > 0 else 0.0

    top = get_tickets_top_buyers(5)
    if top:
        lines_top = ""
        for i, (uid, cnt, uname) in enumerate(top, 1):
            lines_top += _format_top_buyer_line(i, int(uid), int(cnt), str(uname or ""))
    else:
        lines_top = "\n   <i>поки немає продажів</i>"

    return f"""<b>🎫 СТАТИСТИКА КВИТКІВ</b> (мінідодаток)

<b>◼ ПУЛ</b>
• Продано: <b>{sold}</b>

<b>◼ УЧАСНИКИ</b>
• Унікальних покупців: <b>{buyers}</b>

<b>◼ ТОП-5 ЗА КІЛЬКІСТЮ КВИТКІВ</b>{lines_top}

<i>Оновлено: {datetime.now().strftime('%d.%m.%Y %H:%M')}</i>"""


def generate_tickets_export(fmt: str) -> tuple[str, int]:
    """
    fmt: txt | csv | xlsx
    Повертає (абсолютний шлях до файлу, кількість квитків у файлі).
    """
    rows, columns = get_all_tickets_for_export()
    df = pd.DataFrame(rows, columns=columns)
    bot_root = Path(__file__).resolve().parent.parent
    stamp = datetime.now().strftime("%d.%m.%Y_%H-%M")

    if fmt == "txt":
        path = bot_root / f"tickets_export_{stamp}.txt"
        df.to_csv(path, sep="\t", index=False, encoding="utf-8")
    elif fmt == "csv":
        path = bot_root / f"tickets_export_{stamp}.csv"
        df.to_csv(path, index=False, encoding="utf-8-sig")
    elif fmt == "xlsx":
        path = bot_root / f"tickets_export_{stamp}.xlsx"
        header_fill = PatternFill(start_color="4472C4", end_color="4472C4", fill_type="solid")
        header_font = Font(bold=True, color="FFFFFF", size=11)
        header_alignment = Alignment(horizontal="center", vertical="center")
        thin_border = Border(
        ...
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            df.to_excel(writer, sheet_name="Квитки", index=False)
            ws = writer.sheets["Квитки"]
            for cell in ws[1]:
                cell.fill = header_fill
                cell.font = header_font
                cell.alignment = header_alignment
            ...
            for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=len(columns)):
                for cell in row:
                    cell.border = thin_border
                    cell.alignment = Alignment(vertical="center")
            widths = {"A": 8, "B": 14, "C": 22, "D": 12, "E": 22}
